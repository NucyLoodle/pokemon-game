...
def cpu_turn():
    ...
    if request.form['launchBattle']:
        ...
        cpu_pokemon_dict = dict(name = f"{cpu_pokemon_name}", move = f"{cpu_move}", damage = cpu_damage)
        user_pokemon_dict = dict(name = f"{user_pokemon_name}", move = f"{user_pokemon['moves']}", hp = user_pokemon['hp'])
        cpu_move_result = []
